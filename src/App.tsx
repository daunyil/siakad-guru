...
import { BarcodeAttendanceScannerModal } from './components/barcode/BarcodeAttendanceScannerModal';
import { StudentCardGeneratorModal } from './components/barcode/StudentCardGeneratorModal';
import { TeacherProfileSwitcherModal } from './components/modals/TeacherProfileSwitcherModal';
import { exportToExcel, exportToWord, triggerPrint } from './utils/exporters';
import { Plus, Edit3, CheckCircle2, RotateCcw, FolderTree, BarChart3, BookOpenCheck, Calendar, BookOpen, Award, FileText, Menu, X, ChevronLeft, ChevronRight, User, Building, Sparkles, LayoutDashboard, Zap, Users, Search, Calculator, Layers, ShieldCheck, FileSpreadsheet, CheckSquare, Star, ChevronDown, AlertTriangle, Scan, CreditCard } from 'lucide-react';
...
export default function App() {
  ...
  // Role selection state ('subject_teacher' | 'guru_piket')
  const [currentRole, setCurrentRole] = useState<UserRole>(() => loadStorageData('userRole', 'subject_teacher'));
  ...
  useEffect(() => { saveStorageData('userRole', currentRole); }, [currentRole]);

  // Quick edit modal, Express KBM, and Teacher Profile Switcher modal state
  ...
  // Teacher Profile Switcher Handlers
  const handleSelectTeacherProfile = (selected: TeacherProfile) => {
    setTeacher(selected);
    // Find if there is an assignment matching this teacher
    const matchingAsg = assignments.find((a) => a.teacherId === selected.id);
    if (matchingAsg) {
      setSelectedAssignmentId(matchingAsg.id);
    } else {
      const subjectAsg = assignments.find((a) => a.subject.toLowerCase() === selected.subject.toLowerCase());
      if (subjectAsg) {
        setSelectedAssignmentId(subjectAsg.id);
      } else if (assignments.length > 0) {
        const newAsg: TeachingAssignment = {
          id: `asg-${selected.id}-${rosters[0]?.classId || '7a'}-${Date.now()}`,
          classId: rosters[0]?.classId || 'cls-7a',
          classLabel: rosters[0]?.classLabel || 'VII-A',
  ...
  const handleUpdateTeacherProfile = (updated: TeacherProfile) => {
    ...
    // Also update any assignments for this teacher with new subject
    setAssignments((prev) =>
      prev.map((a) => (a.teacherId === updated.id ? { ...a, subject: updated.subject } : a))
    );
  };

  const handleAddTeacherProfile = (newTeacher: TeacherProfile, assignedClassIds: string[]) => {
    setTeachers((prev) => [...prev, newTeacher]);
    
    // Create assignments for selected classes
    const newAssignments: TeachingAssignment[] = assignedClassIds.map((clsId) => {
      const roster = rosters.find((r) => r.classId === clsId);
      return {
        id: `asg-${newTeacher.id}-${clsId}-${Date.now()}`,
        classId: clsId,
        classLabel: roster?.classLabel || 'Kelas',
  ...
  // All state object for export/backup
  const allStateData = useMemo(() => ({
    ...
    lessonSessions,
    teachingJournals,
  }), [school, teacher, teachers, year, assignments, rosters, currentRole, gradeBook, attendanceRecords, lessonSessions, teachingJournals]);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 font-sans flex flex-col md:flex-row">
      ...
      <AppSidebar
        ...
        onOpenCardGenerator={() => setIsCardGeneratorOpen(true)}
        onOpenTeacherSwitcher={() => setIsTeacherSwitcherOpen(true)}
      />
        ...
        <header className="bg-slate-900 text-white border-b border-slate-800 sticky top-0 z-30 shadow-md no-print px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <button
              ...
              className="md:hidden p-2 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg transition-colors border border-slate-700 shrink-0"
            ...
            <div className="min-w-0">
              <div className="text-[10px] font-semibold tracking-wider text-slate-400 flex items-center gap-1.5 truncate">
              ...
              <h2 className="text-sm md:text-base font-bold text-white leading-tight truncate">
                {mainModule === 'dashboard'
                  ? currentRole === 'guru_piket'
                    ? 'Command Center Guru Piket & Ketertiban'
          ...
          <div className="flex items-center gap-2 shrink-0">
            {/* Guru & Mapel Quick Switcher Pill */}
            <button
              type="button"
              onClick={() => setIsTeacherSwitcherOpen(true)}
              className="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-800/90 hover:bg-slate-700 border border-slate-700 text-slate-200 hover:text-white rounded-xl text-xs font-bold transition-all shadow-2xs group cursor-pointer"
              title="Klik untuk memilih profil guru lain atau mengubah mapel"
            >
              <div className="w-5 h-5 rounded-full bg-blue-600 text-white font-black text-[10px] flex items-center justify-center">
                {teacher.name.charAt(0)}
              </div>
              <span className="hidden sm:inline max-w-[120px] md:max-w-[160px] truncate text-amber-300 font-extrabold">
                {teacher.name}
              </span>
              <span className="hidden md:inline text-slate-400 font-normal">· {teacher.subject}</span>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl font-bold text-xs border transition-all ${
                currentRole === 'guru_piket'
                  ? 'bg-rose-950/80 text-rose-300 border-rose-600/80 hover:bg-rose-900'
                  : 'bg-blue-950/80 text-blue-300 border-blue-600/80 hover:bg-blue-900'
              }`}
              title="Klik untuk Beralih Peran (Guru Mapel / Guru Piket)"
            >
              <span className={`w-2 h-2 rounded-full inline-block ${
                currentRole === 'guru_piket' ? 'bg-rose-400 animate-pulse' : 'bg-blue-400'
              }`} />
              <span className="hidden lg:inline">Peran:</span>
              <span className="font-extrabold">{currentRole === 'guru_piket' ? 'Piket' : 'Mapel'}</span>
            </button>
            ...
            <button
              ...
              title="Buka Scanner Barcode / QR Siswa"
            >
            ...
            <span className="hidden xl:inline-block px-3 py-1.5 bg-slate-800 border border-slate-700 text-slate-300 text-xs font-medium rounded-xl">
      ...
      {/* ── STUDENT CARD & BARCODE GENERATOR MODAL ── */}
      <StudentCardGeneratorModal
        isOpen={isCardGeneratorOpen}
        onClose={() => setIsCardGeneratorOpen(false)}
        rosters={rosters}
        school={school}
        initialClassId={currentAssignment.classId}
      />

      {/* ── TEACHER PROFILE SWITCHER & MULTI-GURU MODAL ── */}
      <TeacherProfileSwitcherModal
        isOpen={isTeacherSwitcherOpen}
        onClose={() => setIsTeacherSwitcherOpen(false)}
        currentTeacher={teacher}
        teachers={teachers}
        onSelectTeacher={handleSelectTeacherProfile}
        onUpdateTeacher={handleUpdateTeacherProfile}
        onAddTeacher={handleAddTeacherProfile}
        assignments={assignments}
        rosters={rosters}
      />
    </div>
