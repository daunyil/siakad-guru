...
interface P5KopEditorProps {
  kop: KopData;
  setKop: React.Dispatch<React.SetStateAction<KopData>>;
  onClose: () => void;
}

export const P5KopEditor: React.FC<P5KopEditorProps> = ({
  kop,
  setKop,
  onClose,
}) => {
  return (
    <div className="bg-white border-2 border-amber-200 rounded-2xl p-5 shadow-xs space-y-4 no-print">
      <div className="flex items-center justify-between border-b pb-2">
        <h3 className="text-xs font-bold text-slate-900 uppercase">
          Pengaturan Kop Dokumen & Pengesahan Modul P5
        </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
        ...
        <div>
          <label className="block font-bold text-slate-700 mb-1">Koordinator / Guru P5</label>
          <input
            type="text"
        ...
        <div>
          <label className="block font-bold text-slate-700 mb-1">NIP Koordinator P5</label>
          <input
            type="text"
