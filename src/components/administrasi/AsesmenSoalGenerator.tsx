...
import { masterBukuSiswaData, findBukuSiswaSubject } from '../../data/bukuSiswaData';
import {
  authenticPancasilaQuestionBank,
  getAuthenticQuestionsForClass,
} from '../../data/bukuSiswaSoalData';
...
export const AsesmenSoalGenerator: React.FC<AsesmenSoalGeneratorProps> = ({
  ...
  const [targetClass, setTargetClass] = useState<'VII' | 'VIII' | 'IX'>(() => {
    if (selectedClassLabel?.toUpperCase().includes('VIII') || teacher.teachingClass?.includes('8')) return 'VIII';
    if (selectedClassLabel?.toUpperCase().includes('IX') || teacher.teachingClass?.includes('9')) return 'IX';
    return 'VII';
  });
  ...
  const activeBukuSiswaSubject = useMemo(() => {
    return findBukuSiswaSubject(currentSubject.subjectName, targetClass) || masterBukuSiswaData[0];
  }, [currentSubject.subjectName, targetClass]);
  ...
  // Initial Questions Data mapped from Authentic Textbook Question Bank
  const initialQuestions = useMemo(() => {
    const textbookQuestions = getAuthenticQuestionsForClass(targetClass, semester, assessmentType, selectedBabFilter);
    if (textbookQuestions.length > 0) {
      return textbookQuestions;
    }
    ...
    currentSubject.elements.forEach((elem) => {
      elem.tpList.forEach((tp) => {
        ...
        questionsList.push({
          ...
          indicator: `Disajikan stimulus narasi, peserta didik dapat mengidentifikasi konsep utama ${tp.title.toLowerCase()} dengan tepat.`,
          cognitiveLevel: 'L2 (Penerapan)',
          questionType: 'PG',
          stemText: `Perhatikan wacana buku teks pada materi ${elem.name} berikut!\n\nSeorang siswa menelaah bahwa dalam materi "${tp.title}", terdapat nilai luhur Pancasila yang harus ditegakkan.\n\nPrinsip utama yang paling sesuai dengan wacana tersebut adalah...`,
          options: {
            a: `Meningkatkan partisipasi aktif dan tanggung jawab pribadi serta kelompok`,
            b: `Menyerahkan seluruh tugas dan keputusan kepada orang lain saja`,
            c: `Mengabaikan aturan dan kesepakatan bersama yang telah ditetapkan`,
            d: `Mencari keuntungan pribadi tanpa mempedulikan kepentingan bersama`,
          },
          keyAnswer: 'A',
  ...
  }, [targetClass, semester, assessmentType, selectedBabFilter, currentSubject]);

  const [questions, setQuestions] = useState<QuestionCardItem[]>(initialQuestions);

  // When filters change, sync questions
  useEffect(() => {
    const fresh = getAuthenticQuestionsForClass(targetClass, semester, assessmentType, selectedBabFilter);
    if (fresh.length > 0) {
      setQuestions(fresh);
    }
  }, [targetClass, semester, assessmentType, selectedBabFilter]);
  ...
  const handleAddQuestion = () => {
    ...
    const newQ: QuestionCardItem = {
      ...
      elementName: currentSubject.elements[0]?.name || 'Pancasila',
      indicator: 'Disajikan stimulus kasus kehidupan nyata, peserta didik dapat menentukan solusi yang tepat.',
      cognitiveLevel: 'L2 (Penerapan)',
      questionType: 'PG',
      stemText: 'Tuliskan stimulus narasi dan rumusan naskah soal di sini...',
      options: {
        a: 'Pilihan Jawaban A',
        b: 'Pilihan Jawaban B',
        c: 'Pilihan Jawaban C',
  ...
  // Reset & Re-generate Questions from Textbook
  const handleReloadQuestions = () => {
    const fresh = getAuthenticQuestionsForClass(targetClass, semester, assessmentType, selectedBabFilter);
    if (fresh.length > 0) {
      setQuestions(fresh);
      showToast(`✨ Soal berhasil disinkronkan dengan Buku Teks Pendidikan Pancasila Kelas ${targetClass}!`);
    } else {
      showToast('Soal dimuat ulang.');
    }
  };
  ...
  const handlePrint = () => {
    smartPrint({
      ...
      docTitle: `Naskah Soal & Kisi-Kisi (${assessmentType}) - Pendidikan Pancasila Kelas ${targetClass}`,
    ...
    <div className="space-y-6 font-sans">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-2.5 py-1 bg-purple-500/20 text-purple-300 border border-purple-400/30 rounded-full text-[11px] font-bold uppercase tracking-wider flex items-center gap-1.5">
                ...
                Buku Teks Utama Kemendikbudristek RI
              </span>
            ...
            <h2 className="text-xl md:text-2xl font-bold text-white tracking-tight">
              Bank Soal & Kisi-Kisi Asesmen Pendidikan Pancasila
            </h2>
            <p className="text-xs md:text-sm text-slate-300 max-w-3xl leading-relaxed">
              Otomatiskan penyusunan dokumen asesmen standar BSKAP Kemendikbudristek RI: Tabel Kisi-Kisi Soal, Kartu Soal Lengkap, Naskah Ujian Siswa (PG HOTS & Uraian Analitis), serta Lembar Kunci Jawaban dan Pedoman Penskoran.
            </p>
            ...
            <div className="pt-2 flex flex-wrap items-center gap-3 text-xs">
              ...
              <button
                ...
                className="px-2.5 py-1 bg-purple-700/80 hover:bg-purple-600 text-white rounded-lg border border-purple-400/40 text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer"
              >
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 shrink-0">
            <button
              ...
              className="px-3.5 py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-bold rounded-xl text-xs transition-all shadow-md flex items-center justify-center gap-1.5 cursor-pointer"
            >
              <RefreshCw className="w-4 h-4 text-amber-200" />
              <span>Muat Ulang Soal Buku Siswa</span>
            </button>

            <button
              onClick={handlePrint}
              className="px-3.5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl text-xs transition-all shadow-md flex items-center justify-center gap-1.5 cursor-pointer"
            >
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4 no-print">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 text-xs">
          <div>
            <label className="block font-bold text-slate-700 mb-1">1. Jenjang / Kelas SMP:</label>
            <select
              value={targetClass}
              onChange={(e) => setTargetClass(e.target.value as 'VII' | 'VIII' | 'IX')}
              className="w-full bg-slate-50 border border-slate-300 text-slate-900 font-bold rounded-xl px-3 py-2 focus:ring-2 focus:ring-purple-500 cursor-pointer"
            >
              <option value="VII">Kelas VII (Buku Kemendikbud)</option>
              <option value="VIII">Kelas VIII (Buku Kemendikbud)</option>
              <option value="IX">Kelas IX (Buku Kemendikbud)</option>
            </select>
          </div>

          <div>
            <label className="block font-bold text-slate-700 mb-1">2. Jenis Asesmen:</label>
            <select
          ...
          <div>
            <label className="block font-bold text-slate-700 mb-1">3. Bab Buku Siswa:</label>
            <select
              value={selectedBabFilter}
              onChange={(e) => setSelectedBabFilter(e.target.value)}
              className="w-full bg-slate-50 border border-slate-300 text-slate-900 font-bold rounded-xl px-3 py-2 focus:ring-2 focus:ring-purple-500 cursor-pointer"
            >
              <option value="all">Semua Bab (Sesuai Semester)</option>
              {activeBukuSiswaSubject?.babList.map((b) => (
                <option key={b.id} value={b.id}>
                  {b.title} (Sem {b.semester})
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            <label className="block font-bold text-slate-700 mb-1">4. Semester:</label>
            <select
              value={semester}
          ...
          <div>
            <label className="block font-bold text-slate-700 mb-1">5. Alokasi Waktu:</label>
            <input
              type="text"
              value={timeAllocation}
              onChange={(e) => setTimeAllocation(e.target.value)}
              className="w-full px-3 py-2 border border-slate-300 rounded-xl font-bold focus:ring-2 focus:ring-purple-500 bg-slate-50"
            />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4">
          ...
          <button
            ...
            className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-sm flex items-center gap-1.5 cursor-pointer"
          >
      ...
      <div className="document-page bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm print:p-0 print:border-none print:shadow-none space-y-6 font-sans">
        ...
        <div className="border-b-4 border-double border-black pb-4 text-center space-y-1 font-serif">
          ...
          <div className="text-xs font-bold text-purple-950 uppercase tracking-wider font-sans">
            PENDIDIKAN PANCASILA KELAS {targetClass} — {assessmentTypeLabel}
          </div>
          <p className="text-[11px] font-sans text-slate-600">
            Tahun Pelajaran {year.label} | Kelas {targetClass} Semester {semester === 1 ? 'Ganjil' : 'Genap'} | Alokasi Waktu: {timeAllocation} | Rujukan Buku Siswa Kemendikbudristek RI
          </p>
        </div>
              ...
              {questions.map((q) => (
                <div key={q.id} className="border-2 border-black rounded-xl p-4 bg-white space-y-3 font-sans text-xs break-inside-avoid">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 bg-slate-50 p-2.5 rounded-lg border border-slate-300">
                    <div>
                      <p><strong>Mata Pelajaran:</strong> Pendidikan Pancasila</p>
                  ...
                  <div className="border-t border-slate-300 pt-2 space-y-1">
                    ...
                    <p className="text-slate-700 italic whitespace-pre-line">{q.scoringGuide}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'naskah' && (
          <div className="space-y-6">
            <div className="border-2 border-black p-4 rounded-xl text-xs font-serif font-bold text-slate-900 grid grid-cols-2 gap-4">
              ...
              <div className="text-right">
                <p>MATA PELAJARAN: PENDIDIKAN PANCASILA</p>
                <p>HARI / TANGGAL: .....................................................</p>
              </div>
            </div>
            ...
            <div className="space-y-6 font-serif text-sm text-slate-950">
              <div className="font-bold border-b border-black pb-1 uppercase">
                PETUNJUK: PILIHLAH JAWABAN YANG PALING TEPAT DENGAN MEMBERI TANDA SILANG (X) PADA HURUF A, B, C, ATAU D ATAU TULISKAN JAWABAN URAIAN DENGAN LENGKAP!
              </div>
              ...
              {questions.map((q) => (
                <div key={q.id} className="space-y-2 break-inside-avoid">
                  ...
                  {q.options ? (
                    ...
                    <div className="pl-6 border-2 border-dashed border-slate-400 rounded-xl p-3 min-h-[90px] bg-slate-50/30">
        ...
        <div className="pt-8 grid grid-cols-2 gap-8 text-xs font-serif font-medium text-slate-900 break-inside-avoid">
          ...
          <div className="text-center space-y-16">
            <div>
              ...
              <p className="font-bold uppercase text-purple-950">Pendidikan Pancasila</p>
            </div>
            <div>
