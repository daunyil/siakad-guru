...
export const PiketBkEkstraGenerator: React.FC<PiketBkEkstraGeneratorProps> = ({
  ...
  const handleQuickAddAbsence = (keterangan: 'Sakit' | 'Izin' | 'Alpa' | 'Bolos/Cabut') => {
    if (!newAbsenceInput.nama.trim()) {
      showToast('⚠️ Ketik atau pilih nama siswa terlebih dahulu!');
      return;
    }
    const studentName = newAbsenceInput.nama.trim();
    setClassAttendance((prev) =>
      prev.map((c) => {
        if (c.kelas === newAbsenceInput.kelas) {
          const newAbs = [
    ...
    setNewAbsenceInput((prev) => ({ ...prev, nama: '', nisn: '', alasan: '' }));
    showToast(`✓ [${keterangan}] ${studentName} (Kelas ${newAbsenceInput.kelas}) tersimpan!`);
  };
  ...
  const [piketSubView, setPiketSubView] = useState<'presensi' | 'pelanggaran' | 'rekap'>('presensi');
  const [showAdvancedPiketFields, setShowAdvancedPiketFields] = useState(false);
  const [isSection1Open, setIsSection1Open] = useState(true);
  const [isSection2Open, setIsSection2Open] = useState(true);
  const [isJournalPiketFinished, setIsJournalPiketFinished] = useState(false);
  const [showDocumentPreview, setShowDocumentPreview] = useState(false);
  const [isHeaderCollapsed, setIsHeaderCollapsed] = useState(false);
  const [isCustomPelanggaran, setIsCustomPelanggaran] = useState(false);
  ...
  const [newPiket, setNewPiket] = useState({
    ...
    poin: 5,
    tindakanPiket: 'Pembinaan lisan & izin masuk kelas',
    statusDisposisi: 'Selesai di Piket' as RecordPelanggaranPiket['statusDisposisi'],
  });

  const handleSelectViolationDropdown = (selectedVal: string) => {
    if (selectedVal === '__CUSTOM__') {
      setIsCustomPelanggaran(true);
      setNewPiket((prev) => ({
        ...prev,
        jenisPelanggaran: '',
      }));
    ...
    setIsCustomPelanggaran(false);
    const rule = violationRules.find((r) => r.id === selectedVal || r.nama === selectedVal);
    if (rule) {
      ...
      showToast(`Dipilih: "${rule.nama}" (+${rule.poin} Poin)`);
    }
  };

  const handleAddPiket = () => {
    if (!newPiket.namaSiswa || !newPiket.jenisPelanggaran) {
      showToast('Pilih / isi nama siswa dan jenis pelanggaran terlebih dahulu!');
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── HEADER BANNER (Bisa diciutkan agar layar ringkas) ── */}
      <div className="bg-gradient-to-r from-indigo-950 via-slate-900 to-sky-950 text-white rounded-2xl p-4 md:p-6 shadow-md border border-slate-800 relative overflow-hidden transition-all">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div className="space-y-1.5 flex-1">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-2.5 py-0.5 bg-sky-500/20 text-sky-300 border border-sky-400/30 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5">
                <ShieldAlert className="w-3.5 h-3.5 text-sky-400" />
                Administrasi Guru Piket & Ketertiban
              </span>
              <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
            ...
            <div className="flex items-center justify-between">
              <h2 className="text-lg md:text-xl font-bold text-white tracking-tight">
                Jurnal Guru Piket, Layanan BK, & Ekstrakurikuler
              </h2>
            </div>
            
            {!isHeaderCollapsed && (
              <p className="text-xs text-slate-300 max-w-3xl leading-relaxed">
                Catat presensi harian siswa per kelas, data keterlambatan/pelanggaran langsung, disposisi ke BK / Wali Kelas, serta cetak lembar rekapitulasi resmi.
              </p>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            <button
              ...
              className="px-3 py-1.5 bg-slate-800/90 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-xl text-xs font-semibold transition-all flex items-center gap-1"
              title={isHeaderCollapsed ? 'Tampilkan deskripsi' : 'Ciutkan banner'}
            >
              {isHeaderCollapsed ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronUp className="w-3.5 h-3.5" />}
              <span>{isHeaderCollapsed ? 'Perluas Info' : 'Ciutkan'}</span>
            </button>
            <button
            ...
            >
              ...
              <span>{isEditingKop ? 'Tutup Kop' : 'Atur Kop'}</span>
            </button>
            <button
              onClick={handlePrint}
              className="px-3.5 py-1.5 bg-sky-600 hover:bg-sky-500 text-white rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-1.5"
            >
      ...
      {activeTab === 'piket' && (
        <div className="space-y-4 md:space-y-6 pb-16 md:pb-0">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 md:gap-3 no-print">
            <div className="bg-white p-2.5 md:p-3 rounded-2xl border border-slate-200 shadow-xs text-center">
              <span className="text-[9px] md:text-[10px] font-extrabold uppercase text-slate-500">Total Siswa</span>
              <div className="text-lg md:text-xl font-black text-slate-900 mt-0.5">
                {classAttendance.reduce((acc, c) => acc + c.totalSiswa, 0)}
              </div>
            </div>

            <div className="bg-emerald-50 p-2.5 md:p-3 rounded-2xl border border-emerald-200 shadow-xs text-center">
              <span className="text-[9px] md:text-[10px] font-extrabold uppercase text-emerald-700">Hadir</span>
              <div className="text-lg md:text-xl font-black text-emerald-800 mt-0.5">
                {classAttendance.reduce((acc, c) => acc + c.hadir, 0)}
              </div>
            </div>

            <div className="bg-sky-50 p-2.5 md:p-3 rounded-2xl border border-sky-200 shadow-xs text-center">
              <span className="text-[9px] md:text-[10px] font-extrabold uppercase text-sky-700">Sakit (S)</span>
              <div className="text-lg md:text-xl font-black text-sky-800 mt-0.5">
                {classAttendance.reduce((acc, c) => acc + c.sakit, 0)}
              </div>
            </div>

            <div className="bg-amber-50 p-2.5 md:p-3 rounded-2xl border border-amber-200 shadow-xs text-center">
              <span className="text-[9px] md:text-[10px] font-extrabold uppercase text-amber-700">Izin (I)</span>
              <div className="text-lg md:text-xl font-black text-amber-800 mt-0.5">
                {classAttendance.reduce((acc, c) => acc + c.izin, 0)}
              </div>
            </div>

            <div className="bg-rose-50 p-2.5 md:p-3 rounded-2xl border border-rose-200 shadow-xs text-center">
              <span className="text-[9px] md:text-[10px] font-extrabold uppercase text-rose-700">Alpa / Bolos</span>
              <div className="text-lg md:text-xl font-black text-rose-800 mt-0.5">
                {classAttendance.reduce((acc, c) => acc + c.alpa, 0)}
              </div>
            </div>

            <div className="bg-indigo-50 p-2.5 md:p-3 rounded-2xl border border-indigo-200 shadow-xs text-center">
              <span className="text-[9px] md:text-[10px] font-extrabold uppercase text-indigo-700">% Kehadiran</span>
          ...
          {/* ── MODE CEPAT PIKET: SUB-NAVIGATOR (PRESENSI / PELANGGARAN / REKAP CETAK) ── */}
          <div className="bg-slate-900 p-1.5 rounded-2xl shadow-md no-print border border-slate-800">
            <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none">
              <button
                type="button"
                onClick={() => setPiketSubView('presensi')}
                className={`flex-1 min-w-[130px] py-2.5 px-3 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-2 ${
                  piketSubView === 'presensi'
                    ? 'bg-emerald-600 text-white shadow-md'
                    : 'text-slate-300 hover:text-white hover:bg-slate-800'
                }`}
              >
                <UserCheck className="w-4 h-4 text-emerald-300" />
                <span>1. Presensi Harian</span>
                <span className="px-1.5 py-0.5 bg-black/30 text-emerald-200 rounded-full text-[10px]">
                  {classAttendance.reduce((acc, c) => acc + c.absenStudents.length, 0)}
                </span>
              </button>

              <button
                type="button"
                onClick={() => setPiketSubView('pelanggaran')}
                className={`flex-1 min-w-[130px] py-2.5 px-3 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-2 ${
                  piketSubView === 'pelanggaran'
                    ? 'bg-red-600 text-white shadow-md'
                    : 'text-slate-300 hover:text-white hover:bg-slate-800'
                }`}
              >
                <ShieldAlert className="w-4 h-4 text-red-300" />
                <span>2. Catat Pelanggaran</span>
                <span className="px-1.5 py-0.5 bg-black/30 text-red-200 rounded-full text-[10px]">
                  {piketRecords.length}
                </span>
              </button>
              ...
              <button
                type="button"
                onClick={() => setPiketSubView('rekap')}
                className={`flex-1 min-w-[120px] py-2.5 px-3 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-2 ${
                  piketSubView === 'rekap'
              ...
              >
                ...
                <span>3. Rekap & Cetak</span>
              </button>
            </div>
          </div>

          {/* ════════════════════════════════════════════════════════════════════════════
              SUB-VIEW 1: PRESENSI KETIDAKHADIRAN HARIAN (ULTRA FAST MOBILE ENTRY)
             ════════════════════════════════════════════════════════════════════════════ */}
          {piketSubView === 'presensi' && (
            <div className="space-y-4 no-print animate-fadeIn">
              <div className="bg-white border-2 border-emerald-300 rounded-2xl p-4 md:p-5 shadow-sm space-y-4">
                <div className="flex flex-wrap items-center justify-between border-b pb-2 gap-2">
                  <div>
                    <h3 className="text-xs md:text-sm font-extrabold text-emerald-900 uppercase flex items-center gap-2">
                      <UserCheck className="w-4 h-4 text-emerald-600" />
                      Input Cepat Ketidakhadiran Siswa (Piket)
                    </h3>
                    <p className="text-[11px] text-slate-500">
                      Pilih kelas, ketik nama siswa, lalu tap tombol status untuk simpan instan.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-emerald-50 text-emerald-800 border border-emerald-300 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 hover:bg-emerald-100"
                  >
                ...
                {/* 1. Pilih Kelas Cepat (Horizontal Pills) */}
                <div className="space-y-1.5">
                  <label className="block text-[11px] font-extrabold text-slate-700 uppercase tracking-wider">
                    1. Pilih Kelas yang Sedang Diabsen:
                  </label>
                  <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none">
                    {classAttendance.map((c) => {
                      const isSelected = newAbsenceInput.kelas === c.kelas;
                      const absCount = c.absenStudents.length;
                      return (
                        <button
                          key={c.kelas}
                          type="button"
                          onClick={() => setNewAbsenceInput({ ...newAbsenceInput, kelas: c.kelas })}
                          className={`px-3.5 py-2 rounded-xl font-bold text-xs shrink-0 transition-all border flex items-center gap-1.5 ${
                            isSelected
                              ? 'bg-emerald-600 border-emerald-700 text-white shadow-md scale-[1.02]'
                              : 'bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-700'
                          }`}
                        >
                          <span>Kelas {c.kelas}</span>
                          {absCount > 0 && (
                            <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-extrabold ${
                              isSelected ? 'bg-white text-emerald-800' : 'bg-rose-100 text-rose-800'
                            }`}>
                              {absCount}
                            </span>
                          )}
                ...
                <div className="space-y-1.5">
                  <label className="block text-[11px] font-extrabold text-slate-700 uppercase tracking-wider flex items-center justify-between">
                    <span>2. Nama Siswa (Kelas {newAbsenceInput.kelas}):</span>
                    {newAbsenceInput.nama && (
                      <span className="text-[10px] text-emerald-600 font-bold">Siap disimpan</span>
                    )}
                  </label>
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-3.5 py-3 bg-white border-2 border-slate-300 rounded-xl font-bold text-sm text-slate-900 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all shadow-xs"
                      placeholder="Ketik atau pilih nama siswa..."
                    />
                    {newAbsenceInput.nama && (
                      <button
                        type="button"
                        onClick={() => setNewAbsenceInput({ ...newAbsenceInput, nama: '' })}
                        className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-700"
                        title="Hapus ketikan nama"
                      >
                        <X className="w-4 h-4" />
                      </button>
                    )}
                ...
                {/* 3. 1-Tap Status Action Buttons */}
                <div className="space-y-1.5 pt-1">
                  <label className="block text-[11px] font-extrabold text-slate-700 uppercase tracking-wider">
                    3. Tap Status Kehadiran (1-Klik Simpan Instan):
                  </label>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <button
                      type="button"
                      onClick={() => handleQuickAddAbsence('Sakit')}
                      className="px-3 py-3 bg-sky-600 hover:bg-sky-500 active:scale-95 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all min-h-[46px]"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-3 bg-amber-600 hover:bg-amber-500 active:scale-95 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all min-h-[46px]"
                    >
                      <span className="text-sm">📝</span>
                      <span>+ Izin (I)</span>
                    </button>
                    ...
                    <button
                      ...
                      className="px-3 py-3 bg-rose-600 hover:bg-rose-500 active:scale-95 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all min-h-[46px]"
                    >
                      <span className="text-sm">❌</span>
                      <span>+ Alpa (A)</span>
                    </button>
                    ...
                    <button
                      ...
                      className="px-3 py-3 bg-purple-600 hover:bg-purple-500 active:scale-95 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all min-h-[46px]"
                    >
                      <span className="text-sm">🏃</span>
                      <span>+ Bolos / Cabut</span>
                    </button>
                  </div>
                </div>

                {/* 4. Daftar Siswa Berhalangan yang Sedang Tercatat Hari Ini */}
                <div className="border-t pt-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-extrabold text-slate-800 uppercase tracking-tight flex items-center gap-1.5">
                      <span>Daftar Siswa Berhalangan Hadir Hari Ini</span>
                      <span className="bg-emerald-100 text-emerald-800 px-2 py-0.2 rounded-full text-[10px] font-bold">
                        {classAttendance.reduce((acc, c) => acc + c.absenStudents.length, 0)} Total
                      </span>
                    </span>
                    ...
                    >
                      <span>Lanjut Catat Pelanggaran</span>
                      <ChevronRight className="w-3.5 h-3.5" />
                    </button>
                  </div>

                  <div className="space-y-2">
                    {classAttendance.filter((c) => c.absenStudents.length > 0).length === 0 ? (
                      <div className="bg-emerald-50/50 border border-emerald-200 rounded-xl p-4 text-center text-xs text-emerald-800 italic">
                        Belum ada siswa yang dicatat berhalangan hari ini (Semua kelas masih hadir 100%).
                      </div>
                    ) : (
                      classAttendance
                        .filter((c) => c.absenStudents.length > 0)
                        .map((c) => (
                          <div key={c.kelas} className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 space-y-1.5">
                            <div className="flex items-center justify-between text-xs font-bold text-slate-800 border-b border-slate-200/80 pb-1">
                              <span>Kelas {c.kelas} ({c.absenStudents.length} Siswa)</span>
                              <span className="text-[10px] text-slate-500 font-medium">
                                S: {c.sakit} | I: {c.izin} | A: {c.alpa}
                              </span>
                            </div>
                            <div className="flex flex-wrap gap-1.5">
                              {c.absenStudents.map((s) => (
                                <span
                                  key={s.id}
                                  className={`px-2 py-1 rounded-lg text-xs font-bold border inline-flex items-center gap-1.5 shadow-2xs ${
                                    s.keterangan === 'Sakit'
                                ...
                                >
                                  <span>{s.nama} ({s.keterangan})</span>
                                  <button
                                    type="button"
                                    onClick={() => handleDeleteAbsence(c.kelas, s.id)}
                                    className="p-0.5 hover:bg-red-200 text-red-700 rounded-full font-black ml-1 transition-colors"
                                    title="Hapus siswa ini dari absensi"
                                  >
          ...
          {/* ════════════════════════════════════════════════════════════════════════════
              SUB-VIEW 2: CATAT PELANGGARAN SISWA (ULTRA FAST PRESETS & MOBILE FORM)
             ════════════════════════════════════════════════════════════════════════════ */}
          {piketSubView === 'pelanggaran' && (
            <div className="space-y-4 no-print animate-fadeIn">
              <div className="bg-white border-2 border-red-300 rounded-2xl p-4 md:p-5 shadow-sm space-y-4">
                <div className="flex flex-wrap items-center justify-between border-b pb-2 gap-2">
                  <div>
                    <h3 className="text-xs md:text-sm font-extrabold text-red-900 uppercase flex items-center gap-2">
                      <ShieldAlert className="w-4 h-4 text-red-600" />
                      Catat Pelanggaran & Ketertiban Siswa (Piket)
                    </h3>
                    <p className="text-[11px] text-slate-500">
                      Pilih preset pelanggaran 1-klik untuk auto-bobot poin dan tindakan resmi.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-800 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 hover:bg-slate-700"
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-none">
                      {['VII-A', 'VII-B', 'VIII-A', 'VIII-B', 'IX-A', 'IX-B', 'IX-C'].map((k) => (
                        <button
                          key={k}
                          type="button"
                          onClick={() => setNewPiket({ ...newPiket, kelas: k })}
                          className={`px-2.5 py-1.5 rounded-lg text-xs font-bold shrink-0 transition-all border ${
                            newPiket.kelas === k
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-[11px] font-extrabold text-slate-700 uppercase tracking-wider mb-1">
                      2. Nama Siswa Pelanggar (Kelas {newPiket.kelas}):
                    </label>
                    <div className="relative">
                      <input
                        type="text"
                        ...
                        className="w-full px-3.5 py-2.5 bg-white border-2 border-slate-300 rounded-xl font-bold text-sm text-slate-900 focus:border-red-500 focus:ring-2 focus:ring-red-200 transition-all shadow-xs"
                        placeholder="Ketik atau pilih nama siswa..."
                      />
                      {newPiket.namaSiswa && (
                        <button
                          type="button"
                          onClick={() => setNewPiket({ ...newPiket, namaSiswa: '' })}
                          className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-700"
                        >
                          <X className="w-4 h-4" />
                        </button>
                ...
                {/* 2. Quick Preset Pelanggaran Populer (1-Tap Auto Setup) */}
                <div className="space-y-1.5 pt-1">
                  <label className="block text-[11px] font-extrabold text-slate-700 uppercase tracking-wider">
                    3. Pilih Jenis Pelanggaran (Tap Preset Cepat):
                  </label>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[
                      { label: '🕒 Terlambat Masuk', ruleName: 'Terlambat Masuk Sekolah (< 15 Menit)', point: 5, color: 'border-amber-300 hover:bg-amber-50' },
                      { label: '👔 Atribut / Dasi / Topi', ruleName: 'Atribut Seragam Tidak Lengkap (Dasi/Sabuk/Kaos Kaki/Logo)', point: 5, color: 'border-blue-300 hover:bg-blue-50' },
                      { label: '💇 Rambut / Make-up', ruleName: 'Rambut Gondrong / Dicat Warna-Warni / Make-up Berlebih', point: 5, color: 'border-slate-300 hover:bg-slate-50' },
                      { label: '👟 Sepatu / Kaos Kaki', ruleName: 'Sepatu / Kaos Kaki Tidak Sesuai Ketentuan (Bukan Hitam/Putih)', point: 5, color: 'border-slate-300 hover:bg-slate-50' },
                      { label: '📱 Main HP saat KBM', ruleName: 'Bermain HP / Game saat Pembelajaran Tanpa Izin Guru', point: 10, color: 'border-rose-300 hover:bg-rose-50' },
                      { label: '🏃 Bolos / Keluar Kelas', ruleName: 'Meninggalkan Kelas / Sekolah Tanpa Izin saat KBM (Bolos)', point: 15, color: 'border-orange-300 hover:bg-orange-50' },
                      { label: '🚬 Merokok / Vape', ruleName: 'Membawa / Menghisap Rokok atau Vape di Lingkungan Sekolah', point: 30, color: 'border-red-400 hover:bg-red-50' },
                      { label: '⚔️ Berkelahi / Bullying', ruleName: 'Terlibat Perkelahian Fisik / Tawuran / Bullying Fisik', point: 50, color: 'border-purple-400 hover:bg-purple-50' },
                    ].map((chip) => {
                      const isSelected = newPiket.jenisPelanggaran === chip.ruleName;
                      return (
                        <button
                          key={chip.label}
                          type="button"
                          onClick={() => handleSelectViolationDropdown(chip.ruleName)}
                          className={`p-2.5 rounded-xl text-left border-2 text-xs font-bold transition-all flex flex-col justify-between min-h-[52px] active:scale-95 ${
                            isSelected
                              ? 'bg-red-600 border-red-700 text-white shadow-md scale-[1.02]'
                              : `bg-white text-slate-800 ${chip.color}`
                          }`}
                        >
                          <span>{chip.label}</span>
                          <span className={`text-[10px] mt-1 font-extrabold ${isSelected ? 'text-red-100' : 'text-red-600'}`}>
                            +{chip.point} Poin
                          </span>
                        </button>
                      );
                  ...
                  {/* Dropdown Master & Kustom Trigger */}
                  <div className="pt-2">
                    <select
                      value={isCustomPelanggaran ? '__CUSTOM__' : (violationRules.find((r) => r.nama === newPiket.jenisPelanggaran)?.id || '__CUSTOM__')}
                      onChange={(e) => handleSelectViolationDropdown(e.target.value)}
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl font-bold text-xs text-slate-800"
                    >
                      <option value="">-- Atau Pilih dari Seluruh Master Aturan ({violationRules.length} Aturan) --</option>
                      {violationRules.map((r) => (
                        <option key={r.id} value={r.id}>
                          [{r.kategori}] {r.nama} (+{r.poin} Poin)
                        </option>
                      ))}
                      <option value="__CUSTOM__">✏️ Ketik Manual / Kasus Kustom Lainnya</option>
                    </select>
                    ...
                    {isCustomPelanggaran && (
                      <div className="mt-2">
                        <input
                          ...
                          className="w-full px-3.5 py-2 bg-amber-50 border-2 border-amber-300 rounded-xl font-bold text-xs text-slate-900 placeholder-slate-400"
                          placeholder="Ketik rincian pelanggaran kustom..."
                          autoFocus
                        />
                      </div>
                    )}
                ...
                {/* 3. Ringkasan Poin & Opsi Lanjutan */}
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2 text-xs">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="font-extrabold text-slate-700">Poin:</span>
                      <span className="px-2 py-0.5 bg-red-100 text-red-800 rounded-lg font-black text-xs">
                        +{newPiket.poin} Poin
                      </span>
                      <span className="text-slate-500 font-medium">| {newPiket.kategori}</span>
                    </div>

                    <button
                      type="button"
                      onClick={() => setShowAdvancedPiketFields(!showAdvancedPiketFields)}
                      className="text-slate-600 hover:text-slate-900 font-bold text-[11px] flex items-center gap-1"
                    >
                      <span>{showAdvancedPiketFields ? 'Sembunyikan Opsi Lanjutan' : '⚙️ Opsi Lanjutan (Jam/Tindakan)'}</span>
                      {showAdvancedPiketFields ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                    </button>
                  </div>

                  {showAdvancedPiketFields && (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-2 border-t border-slate-200">
                      <div>
                        <label className="block font-bold text-slate-600 mb-1">Jam / Waktu:</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block font-bold text-slate-600 mb-1">Tindakan Piket:</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block font-bold text-slate-600 mb-1">Disposisi:</label>
                        <select
                          ...
                          className="w-full px-2.5 py-1.5 bg-white border font-bold text-red-950 rounded-lg text-xs"
                        >
                          ...
                          <option value="Rujukan ke Guru BK">🔥 Rujukan ke Guru BK</option>
                        </select>
                      </div>
                    </div>
                  )}
                ...
                {/* 4. Tombol Simpan Pelanggaran (1-Klik) */}
                <button
                  type="button"
                  onClick={handleAddPiket}
                  className="w-full py-3.5 bg-red-600 hover:bg-red-500 active:scale-[0.98] text-white font-extrabold rounded-xl text-sm transition-all shadow-md flex items-center justify-center gap-2 min-h-[48px]"
                >
                  <Plus className="w-5 h-5" />
                  <span>+ Simpan Catatan Pelanggaran Siswa (+{newPiket.poin} Poin)</span>
                </button>

                {/* 5. Daftar Kasus Pelanggaran Hari Ini */}
                <div className="border-t pt-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-extrabold text-slate-800 uppercase tracking-tight flex items-center gap-1.5">
                      <span>Daftar Pelanggaran Hari Ini</span>
                      <span className="bg-red-100 text-red-800 px-2 py-0.2 rounded-full text-[10px] font-bold">
                        {piketRecords.length} Kasus
                      </span>
                    </span>
                    ...
                    >
                      <span>Lihat Rekapitulasi Lengkap</span>
                      <ChevronRight className="w-3.5 h-3.5" />
                    </button>
                  </div>
                  ...
                  <div className="space-y-2">
                    {piketRecords.length === 0 ? (
                      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-center text-xs text-slate-500 italic">
                        Belum ada catatan pelanggaran yang disimpan hari ini (Kondisi Tertib).
                      </div>
                    ) : (
                      piketRecords.map((p) => (
                        <div
                          key={p.id}
                          className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex items-start justify-between gap-2 text-xs hover:bg-slate-100/80 transition-colors"
                        >
                          <div className="space-y-1">
                            <div className="flex items-center gap-1.5">
                              <span className="font-extrabold text-slate-900 text-sm">{p.namaSiswa}</span>
                              <span className="px-1.5 py-0.2 bg-slate-200 text-slate-800 rounded font-bold text-[10px]">
                                Kelas {p.kelas}
                              </span>
                              <span className="px-1.5 py-0.2 bg-red-100 text-red-800 rounded font-extrabold text-[10px]">
                                +{p.poin} Poin
                              </span>
                            </div>
                            ...
                            <div className="text-[11px] text-slate-500 flex items-center gap-2">
                              <span>🕒 {p.jamKe}</span>
                              <span>•</span>
                              <span className="font-medium text-slate-700">{p.tindakanPiket}</span>
                              <span>•</span>
                              <span className={`font-bold ${p.statusDisposisi === 'Rujukan ke Guru BK' ? 'text-red-600' : 'text-emerald-700'}`}>
                                {p.statusDisposisi}
                              </span>
                            </div>
                          </div>
          ...
          {/* ════════════════════════════════════════════════════════════════════════════
              SUB-VIEW 3: REKAPITULASI RESMI & DOKUMEN CETAK
             ════════════════════════════════════════════════════════════════════════════ */}
          {piketSubView === 'rekap' && (
            <div className="space-y-4 animate-fadeIn">
              {/* Kontrol Selesai Mengisi & Cetak Rekap */}
              <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border-2 border-indigo-500/40 rounded-2xl p-4 md:p-5 shadow-lg text-white no-print space-y-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-emerald-500 text-white">
                        ✓ Lembar Rekapitulasi Dokumen Piket
                      </span>
                      <span className="text-xs text-slate-300">
                        {classAttendance.reduce((acc, c) => acc + c.absenStudents.length, 0)} Berhalangan | {piketRecords.length} Pelanggaran
                      </span>
                    </div>

                    <h3 className="text-sm md:text-base font-bold text-white">
                      Dokumen Laporan Piket Harian Siap Dicetak & Ditandatangani
                    </h3>
                    <p className="text-xs text-slate-300">
                      Rekapitulasi otomatis menggabungkan Kehadiran Siswa Per Kelas dan Catatan Pelanggaran Harian.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 shrink-0">
                    ...
                    >
                      ...
                      <span>Edit Pelanggaran</span>
                    </button>
                    <button
                      type="button"
                      onClick={handlePrint}
                      className="px-5 py-2 bg-sky-600 hover:bg-sky-500 text-white rounded-xl text-xs font-bold transition-all shadow-md flex items-center gap-1.5 transform hover:scale-[1.02]"
                    >
              ...
              {/* ── PRINTABLE DOCUMENT CANVASS ── */}
              <div className="bg-white border border-slate-300 rounded-2xl shadow-lg p-6 md:p-12 space-y-8 document-page text-black font-serif text-xs leading-normal">
          ...
          <div className="fixed bottom-3 left-3 right-3 md:hidden z-40 bg-slate-900/95 backdrop-blur-md border border-slate-700 text-white p-2.5 rounded-2xl shadow-2xl flex items-center justify-between gap-2 no-print">
            <div className="flex items-center gap-2 text-xs font-bold pl-1">
              <span className="text-emerald-400">
                {classAttendance.reduce((acc, c) => acc + c.absenStudents.length, 0)} Absen
              </span>
            ...
            <div className="flex items-center gap-1.5">
              {piketSubView !== 'presensi' && (
                <button
                  type="button"
                  onClick={() => setPiketSubView('presensi')}
                  className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-extrabold flex items-center gap-1"
                >
              ...
              {piketSubView !== 'pelanggaran' && (
                <button
                  type="button"
                  onClick={() => setPiketSubView('pelanggaran')}
                  className="px-2.5 py-1.5 bg-red-600 hover:bg-red-500 text-white rounded-xl text-xs font-extrabold flex items-center gap-1"
                >
              ...
              {piketSubView !== 'rekap' && (
                <button
                  type="button"
                  onClick={() => setPiketSubView('rekap')}
                  className="px-2.5 py-1.5 bg-sky-600 hover:bg-sky-500 text-white rounded-xl text-xs font-extrabold flex items-center gap-1"
                >
                  <Printer className="w-3.5 h-3.5" />
                  <span>Cetak</span>
                </button>
